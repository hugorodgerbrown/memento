# 0022. On the pull, one unnamed speaker can count as you
...
**One unnamed speaker counts as you, if you say so.** A Pocket link has a setting, `one_unnamed_speaker_is_me`, which is off by default. When it is on, a recording with exactly one speaker is stored if that speaker carries Pocket's placeholder label: `SPEAKER_00`, or anything matching `speaker[ _-]?\d+`. The log records the reason as "solo, one unnamed speaker (0022)", so these are distinguishable from name-matched ones.

**The rest of Principle 8 holds.**
- Two or more speakers, named or not, are skipped.
- A transcript with no speaker labels is skipped.
- A single speaker that Pocket has named as someone other than you is skipped: only the placeholder counts.
...
- A few recordings that are not the owner may be stored and will need forgetting. This is the only way anyone else's words can be kept, and it needs a single voice, with no one else speaking, while the owner is wearing the Pocket.
